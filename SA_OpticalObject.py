...
class OpticalObjectWorker:
    def __init__(
        ...
        fp.addProperty(
            ...
            "Base",
            "OpticalObject",
            translate("Mirror", "FreeCAD objects to be mirrors or absorbers"),
        ).Base = base
    ...
    def addNewPoperties(self, fp):
        if not hasattr(fp, "Transparency"):
            fp.addProperty(
                ...
                translate(
                    "Mirror",
                    "Percentage of light that passes through the semi transparent mirror",
                ),
            ).Transparency = 0
        ...
        if not hasattr(fp, "collectStatistics"):
            fp.addProperty(
                ...
                "collectStatistics",
                "OpticalObject",
                translate("Mirror", "Count number and coordinates of ray hits"),
            ).collectStatistics = False
...
class LensWorker:
    def __init__(
        ...
        fp.addProperty(
            ...
            "Base",
            "Lens",
            translate("Lens", "FreeCAD objects to be lenses"),
        ).Base = base
        fp.addProperty(
            "App::PropertyFloat",
            "RefractionIndex",
            "Lens",
            translate("Lens", "Refractive Index at 580nm (depends on material)"),
...
class GratingWorker:
    def __init__(
        ...
        fp.addProperty(
            ...
            "Base",
            "Grating",
            translate("Grating", "FreeCAD objects to be diffraction gratings"),
        ).Base = base
        fp.addProperty(
            "App::PropertyFloat",
            "RefractionIndex",
            "Grating",
            translate("Grating", "Refractive Index at 580nm (depends on material)"),
        ).RefractionIndex = RefractionIndex
        fp.addProperty(
            "App::PropertyFloatList",
            "Sellmeier",
        ...
        fp.addProperty(
            ...
            "lpm",
            "Grating",
            translate("Grating", "lines per millimeter"),
        ).lpm = lpm
        fp.addProperty(
            "App::PropertyEnumeration",
            "GratingType",
            "Grating",
            translate("Grating", "reflection or transmission"),
        ).GratingType = [
            "reflection",
            "transmission - diffraction at 2nd surface",
            "transmission - diffraction at 1st surface",
        ]
        fp.addProperty(
            "App::PropertyVector",
            "GratingLinesPlane",
            ...
            translate(
                "Grating",
                "Normal of a hypothetical set of planes that intersect the grating surface, to define the rulings of the grating as these intersection lines",
            ),
        ).GratingLinesPlane = GratingLinesPlane
        fp.addProperty(
            "App::PropertyFloat",
            "order",
            "Grating",
            translate("Grating", "order of diffraction, set by grating"),
        ).order = order
        fp.addProperty(
            "App::PropertyBool",
            "ray_order_override",
            ...
            translate(
                "Grating",
                "if true, order of grating overrides order of ray, if false, ray order is used",
            ),
        ).ray_order_override = ray_order_override
