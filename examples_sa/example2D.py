...
def createSketch_Sketch_Mirror1(doc):
    ...
    Sketch_Mirror1.addGeometry(
        Part.BSplineCurve(
            [
                Vector(0.00, -5.56, 0.00),
                Vector(0.00, -15.18, 0.00),
                Vector(7.19, -16.35, 0.00),
            ]
        )
    )
...
def createSketch_Sketch_Box(doc):
    ...
    Sketch_Box.addGeometry(
        Part.LineSegment(
            Vector(-0.14878900000000073, 11.406045000000002, 0.0),
...
def createSketch_Sketch_Mirror2(doc):
    ...
    Sketch_Mirror2.addGeometry(
        Part.ArcOfCircle(
            Part.Circle(Vector(56.12, -5.69, 0.00), Vector(0.0, 0.0, 1.0), 10.64),
            0.6054907273817108,
            6.799308361137584,
        )
    )
...
def createSketch_Sketch_Lens(doc):
    ...
    Sketch_Lens.addGeometry(
        Part.LineSegment(
            Vector(11.566624790355402, -6.79, 0.0),
            Vector(11.566624790355402, -16.79, 0.0),
        )
    )
    Sketch_Lens.addGeometry(
        Part.ArcOfCircle(
            Part.Circle(Vector(8.25, -11.79, 0.00), Vector(0.0, 0.0, 1.0), 6.00),
            5.298074523841841,
            7.268296090517332,
        )
    )
...
def createSketch_Sketch_Prism(doc):
    ...
    Sketch_Prism.addGeometry(
        Part.LineSegment(
            Vector(21.126467, -13.53020124766231, 0.0),
            Vector(21.126467, -21.018505, 0.0),
        )
    )
    Sketch_Prism.addGeometry(
        Part.LineSegment(
            Vector(21.126467, -21.018505, 0.0),
            Vector(31.579980991773073, -17.27435312383116, 0.0),
        )
    )
    Sketch_Prism.addGeometry(
        Part.LineSegment(
...
def make_optics():
    ...
    ray_beam = sa_OpticsWorkbench.makeRay(
        Vector(0.0, 0.0, 0.0),  # base; change if you need a different origin
...
Gui.addCommand("sa_Example2D", Example2D())
