        ...
        if len(selectedObjList) > 0:
            ...
            for eachObject in selectedObjList:
                ...
                try:
                    if Ray.isOpticalObject(eachObject):
                        ...
                        coords_per_beam[len(coords_per_beam) :] = [
                            getattr(eachObject, attr) for attr in attr_names
                        ]
            ...
            if len(all_coords) > 0:
                ...
                for co in all_coords:
                    if abs(startx - co[0]) > Ray.EPSILON:
                        xpresent = True
                    if abs(starty - co[1]) > Ray.EPSILON:
                        ypresent = True
                    if abs(startz - co[2]) > Ray.EPSILON:
                        zpresent = True
    ...
    def GetResources(self):
        ...
        return {
            ...
            "ToolTip": QT_TRANSLATE_NOOP(
                "RayHits", "Show selected absorbers ray hits in scatter plot"
            ),
        }
